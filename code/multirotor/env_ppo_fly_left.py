...
class SimGym(gym.Env):
    ...
    def _hover_reward(state):
        ...
        y=state[0,1]
        z=state[0,2]
        dis=x**2+2*(10-y)**2+z**2
        mean=0
        sigma=1
        left = 1 / (np.sqrt(2 * math.pi) * np.sqrt(sigma))
        right = np.exp(-(dis - mean)**2 / (2 * sigma))
        reward=100*left * right
         
        roll=state[1,0]
        if roll>1.5 or roll<-1.5:
            reward=-1000
    ...
    def _is_done(self):
        ...
        if roll_angle>1.6 or roll_angle<-1.6:

            done_factor=1    
        # if roll_angle>1.5 or roll_angle<-1.5: 
        return  done_factor

    def step(self, action):
        self.sim.set_controls(t0=action[0], t1=action[0], t2=action[1], t3=action[1])
